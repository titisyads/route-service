...
/**
 * @OA\Info(
 *     title="FleetSync Route Service API",
 *     version="1.0.0",
 *     description="API for managing routes in FleetSync application",
 *     @OA\Contact(
...
class RouteController extends Controller
{
     ...
     *         )
     *     ),
     *     @OA\Response(response=500, description="Failed to fetch routes")
     * )
     */
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Validasi driver
            $driverResponse = Http::get("http://localhost:8001/api/drivers/{$validated['driver_id']}");
            if ($driverResponse->failed() || $driverResponse->json()['status'] !== 'available') {
                return response()->json(['error' => 'Invalid or unavailable driver'], 400);
            }

            // Validasi vehicle
            $vehicleResponse = Http::get("http://localhost:8000/api/vehicles/{$validated['vehicle_id']}");
            if ($vehicleResponse->failed() || $vehicleResponse->json()['status'] !== 'available') {
                return response()->json(['error' => 'Invalid or unavailable vehicle'], 400);
            }
     ...
     *     @OA\Response(response=500, description="Failed to fetch route")
     * )
     */
     ...
     *     @OA\Response(response=404, description="Route not found"),
     *     @OA\Response(response=500, description="Failed to delete route")
     * )
     */
     ...
     *     @OA\Response(response=404, description="Route not found"),
     *     @OA\Response(response=500, description="Failed to update status")
     * )
     */
